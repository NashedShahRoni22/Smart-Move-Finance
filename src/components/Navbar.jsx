          ...
          <div className="absolute left-0 top-0 p-5 min-w-full min-h-screen bg-black/90 lg:hidden gap-8 z-50 flex flex-col">
            {menus.map((m, i) => (
              <div key={i} className="group">
                <Link
                  to={m?.link}
                  className="text-xl md:text-3xl flex justify-between items-center"
                  onClick={() => setShowNav(!showNav)}
                >
                  {m?.name}
                  {m?.child?.length && <BiChevronDown />}
                </Link>
                {m?.child && (
                  <div className="hidden group-hover:flex flex-col gap-2.5 ml-2.5 mt-2.5">
                    {m?.child.map((mc) => (
                      <a
                        href={`/service_details/${mc?.slug}/${mc?.id}`}
